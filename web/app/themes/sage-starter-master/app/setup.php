...
add_action('wp_enqueue_scripts', function () {
    wp_enqueue_script('sage/vendor.js', asset('dist/scripts/vendor.js')->uri(), ['jquery'], null, true);
    wp_enqueue_script('sage/app.js', asset('dist/scripts/app.js')->uri(), ['sage/vendor.js'], null, true);

    wp_add_inline_script('sage/vendor.js', asset('dist/scripts/manifest.js')->contents(), 'before');

    if (is_single() && comments_open() && get_option('thread_comments')) {
        wp_enqueue_script('comment-reply');
    }

    wp_enqueue_style('sage/app.css', asset('dist/styles/app.css')->uri(), null, null, null);
}, 100);
...
add_action('enqueue_block_editor_assets', function () {
    if ($manifest = asset('dist/scripts/manifest.asset.php')->load()) {
        wp_enqueue_script('sage/vendor.js', asset('dist/scripts/vendor.js')->uri(), ...array_values($manifest));
        wp_enqueue_script('sage/editor.js', asset('dist/scripts/editor.js')->uri(), ['sage/vendor.js'], null, true);

        wp_add_inline_script('sage/vendor.js', asset('dist/scripts/manifest.js')->contents(), 'before');
    }

    wp_enqueue_style('sage/editor.css', asset('dist/styles/editor.css')->uri(), false, null);
}, 100);
...
//         $uploadSourceDomain = "https://example.com";
//         $data['url'] = str_replace(get_home_url(), $uploadSourceDomain, $data['url']);
//         $data['baseurl'] = str_replace(get_home_url(), $uploadSourceDomain, $data['baseurl']);
//         return $data;
//     });
// }
...
add_action(
    ...
    function () {
        ...
        $devDomains = ['invsbl', 'vsbl-dev', 'why404'];
...
add_action('phpmailer_init', function ($phpmailer) {
    ...
    if (WP_ENV === "development" || WP_ENV === "staging") {
        $phpmailer->Host = 'kalafioras.serveriai.lt';
        $phpmailer->Port = 587;
        $phpmailer->Username = 'emails@invsbl.lt';
        $phpmailer->Password = 'b32P5tR7p9sbtGnT';
        $phpmailer->SMTPAuth = true;
        $phpmailer->SMTPSecure = 'tls';
    } else {
        // CLIENT MAIL CREDENTIALS
...
add_filter('use_block_editor_for_post_type', function ($is_enabled) {
    // Add necessary logic
    if (!empty($_GET["post"]) && $_GET["post"] === get_option("page_on_front")) return false;
    return $is_enabled;
}, 10, 2);
